-- Atomically turns one paid+attended lesson participant into exactly one receipt.
-- The participant row is locked for the whole transaction. A second caller returns the
-- already-linked receipt instead of reserving another receipt number.
...
begin
  select lp.business_id, lp.student_id, lp.payer_customer_id,
         ...
         l.title, s.display_name, c.display_name, c.phone, c.email
    into v_business, v_student, v_payer,
         ...
         v_lesson_title, v_student_name, v_customer_name, v_customer_phone, v_customer_email
  ...
  v_description := concat('שיעור פרטי - ', coalesce(nullif(trim(v_lesson_title),''), v_student_name));
