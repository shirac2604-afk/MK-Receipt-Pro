-- The operator account remains the owner and is never put into a billable state.

update public.business_members bm
set role = 'owner'
from auth.users u
where bm.user_id = u.id
  and lower(u.email) = 'shirac2604@gmail.com';
...
begin
  ...
  if v_business_id is not null then
    if coalesce(v_is_owner_free,false) then
      ...
      insert into public.business_subscriptions (
        ...
        v_business_id, 'owner_free', 'active', now(), null, null, null, null, now()
      )
      on conflict (business_id) do update
      set plan_key = 'owner_free',
          status = 'active',
