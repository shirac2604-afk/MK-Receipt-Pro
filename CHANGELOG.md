...
## 2026-08-31 — שחזור בטוח מגיבוי מקומי ב־Android

- נוסף לאפליקציה שחזור מגיבוי מקומי לצד יצירה ובדיקה של קובץ גיבוי.
- השחזור מקבל רק קובץ תקין השייך לעסק המחובר, בודק שכל שורה משויכת לאותו עסק ומגביל את גודל הקלט.
- הפעולה אינה מוחקת ואינה דורסת נתונים קיימים: היא מוסיפה רק רשומות שחסרות בענן, בקבוצות מוגבלות.
- קבצי PDF ואסמכתאות עדיין אינם כלולים בקובץ הגיבוי, משום שהם נשמרים בענן בנפרד.
- נוספה בדיקת שחרור סטטית לגבולות השחזור והיא נכללת ב־`release:check`.

## 2026-08-31 — יומן עבודה יומי באפליקציה

- נוסף ב־Android אזור „יום העבודה שלי” ליומן השיעורים: בחירה ישירה מתוך השבוע הקרוב ומעבר ליום הקודם או הבא.
- סדר היום מציג רק את מפגשי היום שנבחר, עם הפעולות הקיימות לנוכחות, סימון תשלום ושמירת סיכום שיעור.
- מסך הגבייה נשאר נפרד ואינו מושפע מסינון היום.
- Windows נשאר עם תצוגת השבוע המלאה הקיימת; שני הממשקים משתמשים באותו יומן ענן ובאותן פעולות מעקב.
- נוספה בדיקת שחרור סטטית לתצוגת היום והיא נכללת ב־`release:check`.

## 2026-08-31 — כרטיס תלמיד מפורט בנייד

- לחיצה על תלמיד באפליקציית Android פותחת כעת כרטיס מרוכז עם פרטי קשר, הורה, תזכורות, מטרות ודגשים.
- הכרטיס מציג שיעורים שהתקיימו, יתרה פתוחה מחושבת רק ממפגשים שהתקיימו וטרם שולמו, ושלושה שיעורים קרובים.
- כל המידע נקרא מאותם נתוני ענן משותפים שכבר מוצגים ב־Windows; לא נוספו שדות פרטיים למכשיר ולא השתנו מסד הנתונים או הרשאות הענן.
- נוספה בדיקת שחרור סטטית שמוודאת את טעינת הנתונים, סיכום הגבייה והקישורים לעריכת הכרטיס וליומן.
