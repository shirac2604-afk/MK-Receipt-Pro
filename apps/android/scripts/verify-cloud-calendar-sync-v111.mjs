...
const reminders=fs.readFileSync("src/students/CloudTeacherReminderPanel.tsx","utf8");
const checks=[
 ...
 ,[panel.includes("<CloudTeacherReminderPanel items={items}"),"cloud schedule exposes teacher device reminders"]
 ,[reminders.includes("source:\"cloud-schedule\"")&&reminders.includes("TeacherReminderNotificationService.sync(reminders)"),"teacher reminders are sourced from the shared schedule"]
];
