...
export type MobileLesson={id:string;seriesId:string|null;kind:"individual"|"group";studentId:string|null;groupId:string|null;title:string;startsAt:string;endsAt:string;cancelled:boolean;lessonSummary:string;homework:string;participants:MobileLessonParticipant[];createdAt:string;updatedAt:string};
...
export const StudentLocalStore={
 ...
 async createLessons(input:{kind:"individual"|"group";studentId?:string|null;groupId?:string|null;title:string;startsAt:string;durationMinutes:number;amountAgorot?:number|null;recurrenceIntervalWeeks:0|1|2;endsOn?:string|null}){
  ...
  const created:MobileLesson[]=[],max=60;for(let i=0;i<max;i++){const occurrence=new Date(start);if(input.recurrenceIntervalWeeks)occurrence.setDate(start.getDate()+i*7*input.recurrenceIntervalWeeks);else if(i>0)break;if(until&&occurrence>until)break;const ts=now();const lesson:MobileLesson={id:id("lesson"),seriesId,kind:input.kind,studentId:input.kind==="individual"?student!.id:null,groupId:input.kind==="group"?group!.id:null,title:input.title.trim().slice(0,160)||"שיעור",startsAt:occurrence.toISOString(),endsAt:new Date(occurrence.getTime()+input.durationMinutes*60000).toISOString(),cancelled:false,lessonSummary:"",homework:"",participants:participantIds.map(studentId=>({studentId,attendance:"scheduled",payment:"unpaid",amountAgorot:amount,paidAt:null})),createdAt:ts,updatedAt:ts};state.lessons.push(lesson);created.push(lesson);}
  await write(state);return created;
 },
 async updateParticipant(lessonId:string,studentId:string,patch:Partial<Pick<MobileLessonParticipant,"attendance"|"payment">>){const state=await read(),lesson=state.lessons.find(x=>x.id===lessonId);if(!lesson)throw new Error("השיעור לא נמצא.");if(lesson.cancelled)throw new Error("לא ניתן לעדכן משתתפים במפגש שבוטל.");const participant=lesson.participants.find(x=>x.studentId===studentId);if(!participant)throw new Error("התלמיד לא נמצא במפגש.");if(patch.attendance)participant.attendance=patch.attendance;if(patch.payment){participant.payment=patch.payment;participant.paidAt=patch.payment==="paid"?now():null;}lesson.updatedAt=now();await write(state);return lesson;},
 async rescheduleLesson(lessonId:string,startsAt:string){const state=await read(),lesson=state.lessons.find(x=>x.id===lessonId);if(!lesson)throw new Error("השיעור לא נמצא.");const start=new Date(startsAt);if(Number.isNaN(start.getTime()))throw new Error("מועד השיעור אינו תקין.");const duration=Math.max(15,new Date(lesson.endsAt).getTime()-new Date(lesson.startsAt).getTime());lesson.startsAt=start.toISOString();lesson.endsAt=new Date(start.getTime()+duration).toISOString();lesson.cancelled=false;lesson.updatedAt=now();await write(state);return lesson;},
