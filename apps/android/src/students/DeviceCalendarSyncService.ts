...
export type CalendarSyncLesson={id:string;title:string;startsAt:string;endsAt:string;cancelled:boolean;kind:"individual"|"group"};
...
function details(lesson:CalendarSyncLesson){return{title:lesson.title,startDate:new Date(lesson.startsAt),endDate:new Date(lesson.endsAt),timeZone:TIME_ZONE,notes:`מפתחות להצלחה - יומן תלמידים\nlesson:${lesson.id}\nסוג: ${lesson.kind==="group"?"קבוצתי":"פרטני"}`};}
...
export const DeviceCalendarSyncService={
 ...
 async sync(lessons:CalendarSyncLesson[]):Promise<DeviceCalendarSyncResult>{
