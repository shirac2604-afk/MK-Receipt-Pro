...
const checks=[
  ...
  [policy.includes("HIBP_RANGE_URL")&&policy.includes('"Add-Padding":"true"')&&policy.includes("crypto.createHash(\"sha1\")"),"leaked-password check uses HIBP k-anonymity with padded range requests"],
  [service.includes("await validateNewPasswordSecurity(current.user.email,newPassword)")&&service.includes("await validateNewPasswordSecurity(recovery.email,newPassword)"),"leaked-password check is enforced for password change and recovery"],
  ...
  [renderer.includes("newPasswordConfirmation")&&renderer.includes('autoComplete="new-password"')&&renderer.includes("לפחות 10 תווים"),"Windows UI confirms, masks and explains the new password policy"],
