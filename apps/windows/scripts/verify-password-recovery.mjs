...
const checks=[
  ...
  [!renderer.includes('autoComplete="one-time-code"')&&!renderer.includes("verifyOtp")&&!renderer.includes("access_token")&&!renderer.includes("refresh_token")&&!renderer.includes("beginPasswordRecovery"),"Windows renderer remains token-blind and does not accept OTP or raw recovery tokens"],
