...
export function StudentsContainer(){
 const[students,setStudents]=useState<StudentWithGuardian[]>([]),[customers,setCustomers]=useState<CustomerRecord[]>([]),[loading,setLoading]=useState(true);
 const load=useCallback(async()=>{setLoading(true);try{const[studentResult,customerResult]=await Promise.all([window.mkApi.students.list(),window.mkApi.customers.list()]);setStudents(unwrap(studentResult));setCustomers(unwrap(customerResult));}finally{setLoading(false)}},[]);
 useEffect(()=>{void load()},[load]);
 async function save(draft:StudentDraft){const price=Number(draft.defaultPriceShekels.replace(",","."));const input:StudentSaveInput={...(draft.id?{id:draft.id}:{}),displayName:draft.displayName,phone:draft.phone,email:draft.email,schoolName:draft.schoolName,schoolGrade:draft.schoolGrade,focusNotes:draft.focusNotes,defaultPriceAgorot:Math.round(price*100),payerCustomerId:draft.payerCustomerId,reminderEnabled:draft.reminderEnabled,guardianName:draft.guardianName,guardianRelationship:draft.guardianRelationship,guardianPhone:draft.guardianPhone,guardianEmail:draft.guardianEmail,guardianReceivesReminders:draft.guardianReceivesReminders};unwrap(await window.mkApi.students.save(input));}
 async function deactivate(studentId:string){if(!window.confirm("להעביר את התלמיד ללא פעיל? היסטוריית השיעורים והקבלות תישמר."))return;unwrap(await window.mkApi.students.deactivate(studentId));await load();}
