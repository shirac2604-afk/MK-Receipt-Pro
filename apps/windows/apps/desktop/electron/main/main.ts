...
// RC clean-start storage namespace.
// This deliberately does not reuse development/test userData.
...
function isTrustedExternalUrl(rawUrl: string): boolean {
  try {
    ...
    return new Set([
      "wa.me",
      "accounts.google.com",
      "noimclnzzuxcszdotmby.supabase.co"
...
function createMainWindow(): void {
  mainWindow = new BrowserWindow({
    ...
    icon: path.join(process.resourcesPath, "installer", "app-icon.ico"),
    webPreferences: {
  ...
  if (devServerUrl === "http://127.0.0.1:5173") void mainWindow.loadURL(devServerUrl);
...
app.whenReady().then(async () => {
  ...
  databaseService.initialize(app.getPath("userData"), app.getPath("documents"), process.resourcesPath);
  const cloudSync=new GoogleDriveSyncService(app.getPath("userData"),databaseService,process.resourcesPath);
  databaseService.setAutomaticCloudSyncHook(()=>cloudSync.schedulePush());
...
app.on("before-quit", () => databaseService.close());
