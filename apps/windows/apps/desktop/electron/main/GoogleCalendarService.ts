...
type Config={clientId:string;lastSyncAt:string|null};
...
export class GoogleCalendarService{
 ...
 private readConfig():Config{try{const raw=JSON.parse(fs.readFileSync(this.configPath,"utf8"));return{clientId:typeof raw?.clientId==="string"?raw.clientId:"",lastSyncAt:typeof raw?.lastSyncAt==="string"?raw.lastSyncAt:null};}catch{return{clientId:"",lastSyncAt:null};}}
 ...
 getStatus():GoogleCalendarStatus{const config=this.readConfig();return{configured:Boolean(config.clientId),connected:Boolean(this.readTokens()),syncing:this.syncing,calendarId:"primary",lastSyncAt:config.lastSyncAt,lastError:this.lastError};}
 setClientId(raw:string):GoogleCalendarStatus{const clientId=raw.trim();if(!/^[A-Za-z0-9._-]+\.apps\.googleusercontent\.com$/.test(clientId))throw new Error("INVALID_GOOGLE_CLIENT_ID");const current=this.readConfig();if(current.clientId&&current.clientId!==clientId){try{fs.rmSync(this.tokenPath,{force:true});}catch{}}this.writeConfig({clientId,lastSyncAt:current.lastSyncAt});this.lastError=null;return this.getStatus();}
 disconnect():GoogleCalendarStatus{try{fs.rmSync(this.tokenPath,{force:true});}catch{}this.lastError=null;return this.getStatus();}
 ...
 async connect():Promise<GoogleCalendarStatus>{
  ...
  try{
   ...
   this.writeTokens({accessToken:data.access_token,refreshToken:typeof data.refresh_token==="string"?data.refresh_token:null,expiresAt:Date.now()+Math.max(60,Number(data.expires_in)||3600)*1000,scope:String(data.scope??SCOPE),tokenType:String(data.token_type??"Bearer")});this.lastError=null;return this.getStatus();
  }catch(error){this.lastError=error instanceof Error?error.message:"GOOGLE_CALENDAR_CONNECT_FAILED";throw error;}finally{server.close();}
 }
 ...
 async syncLessons(rawLessons:LessonRecord[]):Promise<GoogleCalendarSyncResult>{if(this.syncing)throw new Error("GOOGLE_CALENDAR_SYNC_IN_PROGRESS");this.syncing=true;this.lastError=null;try{const unique=[...new Map(rawLessons.map(x=>[x.id,x])).values()];let token=await this.accessToken(),created=0,updated=0,deleted=0,failed=0;for(const lesson of unique){const eventId=this.eventId(lesson.id),base=`${CALENDAR_API}/calendars/primary/events/${encodeURIComponent(eventId)}`;try{if(lesson.status==="cancelled"){let response=await this.request(base,{method:"DELETE"},token);if(response.status===401){token=await this.accessToken(true);response=await this.request(base,{method:"DELETE"},token);}if(response.ok||response.status===404)deleted++;else failed++;continue;}const body=this.eventBody(lesson);let response=await this.request(`${CALENDAR_API}/calendars/primary/events?sendUpdates=none`,{method:"POST",body:JSON.stringify(body)},token);if(response.status===401){token=await this.accessToken(true);response=await this.request(`${CALENDAR_API}/calendars/primary/events?sendUpdates=none`,{method:"POST",body:JSON.stringify(body)},token);}if(response.ok){created++;continue;}if(response.status===409){const patch={...body};delete (patch as any).id;let updatedResponse=await this.request(`${base}?sendUpdates=none`,{method:"PATCH",body:JSON.stringify(patch)},token);if(updatedResponse.status===401){token=await this.accessToken(true);updatedResponse=await this.request(`${base}?sendUpdates=none`,{method:"PATCH",body:JSON.stringify(patch)},token);}if(updatedResponse.ok)updated++;else failed++;}else failed++;}catch{failed++;}}const current=this.readConfig(),lastSyncAt=new Date().toISOString();this.writeConfig({...current,lastSyncAt});return{total:unique.length,created,updated,deleted,failed};}catch(error){this.lastError=error instanceof Error?error.message:"GOOGLE_CALENDAR_SYNC_FAILED";throw error;}finally{this.syncing=false;}}
}
