...
function studentError(error: unknown): ApiResult<never> {
  ...
  if (code === "INVALID_INPUT" || code.startsWith("INVALID_STUDENT") || code === "STUDENT_PAYER_NOT_FOUND_IN_BUSINESS") return apiFailure("INVALID_INPUT", "פרטי התלמיד או ההורה אינם תקינים.", false);
...
function parseSaveInput(raw:any):StudentSaveInput{return {
  ...
  defaultPriceAgorot:Number.isFinite(raw?.defaultPriceAgorot)?Math.trunc(Number(raw.defaultPriceAgorot)):-1,payerCustomerId:"",reminderEnabled:raw?.reminderEnabled!==false,
...
export function registerStudentHandlers(supabaseCloud:SupabaseCloudService,localStore:LocalStudentTestStore):void{
 const cloudStudents=STUDENT_TEST_MODE?null:new StudentManagementCloudService(supabaseCloud);
 ipcMain.handle("students:list",event=>handle(event,undefined,()=>STUDENT_TEST_MODE?localStore.listStudents():cloudStudents!.list()));
 ipcMain.handle("students:save",(event,input)=>handle(event,input,()=>{const parsed=parseSaveInput(input);return STUDENT_TEST_MODE?localStore.saveStudent(parsed):cloudStudents!.save(parsed);}));
 ipcMain.handle("students:deactivate",(event,input)=>handle(event,input,()=>{const studentId=typeof input?.studentId==="string"?input.studentId:"";return STUDENT_TEST_MODE?localStore.deactivateStudent(studentId):cloudStudents!.deactivate(studentId);}));
}
