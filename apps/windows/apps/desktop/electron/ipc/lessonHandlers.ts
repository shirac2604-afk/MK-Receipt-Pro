...
export function registerLessonHandlers(supabaseCloud:SupabaseCloudService):void{
 const lessons=new LessonManagementCloudService(supabaseCloud),lessonNotes=new LessonNotesCloudService(supabaseCloud);
 ...
 ipcMain.handle("lessons:update-participant",(event,input)=>handle(event,input,async()=>{
   ...
   // TEST MODE: student/lesson payments are tracked only. Receipt issuance is intentionally disconnected.
