...
export function registerGoogleCalendarHandlers(service:GoogleCalendarService,store:LocalStudentTestStore):void{
 ...
 ipcMain.handle("google-calendar:sync-now",event=>handle(event,undefined,()=>{const from=new Date();from.setDate(from.getDate()-30);const to=new Date();to.setFullYear(to.getFullYear()+1);return service.syncLessons(store.listLessonsForSync(from.toISOString(),to.toISOString()));}));
}
