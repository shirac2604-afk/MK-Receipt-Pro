...
function errorResult(error: unknown): ApiResult<never> {
  ...
  if (code === "AUTH_RECOVERY_INVALID_LINK"||code === "AUTH_RECOVERY_SESSION_INVALID") return apiFailure("INVALID_INPUT", "קישור השחזור אינו תקף או שפג תוקפו. יש לבקש קישור חדש.", false);
  if (code === "AUTH_RECOVERY_PASSWORD_UPDATE_FAILED") return apiFailure("DATABASE_OPERATION_FAILED", "לא ניתן לעדכן את הסיסמה. יש לבקש קישור חדש ולנסות שוב.", true);
  if (code === "AUTH_RECOVERY_SIGNOUT_FAILED") return apiFailure("DATABASE_OPERATION_FAILED", "הסיסמה עודכנה, אך ניתוק ההתחברויות לא הושלם. יש לבקש קישור חדש ולנסות שוב.", true);
